      ...
      <div className="bg-[#0B132B] border border-[#581C87]/40 p-6 md:p-8 rounded-3xl max-w-4xl w-full shadow-2xl space-y-6 relative overflow-hidden">
        ...
        <div className="text-center space-y-2 max-w-xl mx-auto">
          ...
          <p className="text-xs md:text-sm text-slate-300">
            Escolha o plano ideal para suas metas de agendamentos, vendas e automações sociais.
          </p>
        </div>
          ...
          {plans.map((p) => {
            ...
            return (
              ...
              >
                ...
                <button
                  onClick={() => {
                    if (onSelectPlan) onSelectPlan(p.id as any);
                    onClose();
                  }}
                  disabled={isCurrent}
                ...
                >
                  <span>{isCurrent ? 'Seu Plano Atual' : `Migrar para ${p.name}`}</span>
        ...
        <div className="text-center text-xs text-slate-400 flex items-center justify-center space-x-1.5 pt-2 border-t border-slate-800/60">
          ...
          <span>Sem fidelidade. Cancele ou altere de plano quando quiser diretamente pelo painel.</span>
        </div>
      </div>
    </div>
  );
